...
- **Value**: Must be exactly `"calendly"` (case-sensitive)
- **Required**: Yes
...
switch (connection.type) {
  case 'calendly':
    return new CalendlyOAuthHandler(connection)
  case 'notion':
...
```typescript
type Connection =
  | { type: 'calendly'; clientId: string; clientSecret: string }
...
function connect(conn: Connection) {
  if (conn.type === 'calendly') {
    // TypeScript knows conn has calendly-specific properties
  }
}
```
...
{
  "oneOf": [
    { "$ref": "./calendly-connection.schema.json" },
...
```json
{
  "type": "Calendly" // ❌ Case mismatch
}
```

**Result**: Validation error - "Type must be exactly 'calendly'"

```json
{
  "type": "calendly-connection" // ❌ Wrong format
}
```
