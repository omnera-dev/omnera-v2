...
- **Value**: Must be exactly `"linkedin-ads"` (case-sensitive)
- **Required**: Yes
...
switch (connection.type) {
  case 'linkedin-ads':
    return new LinkedIn AdsOAuthHandler(connection)
  case 'notion':
...
```typescript
type Connection =
  | { type: 'linkedin-ads'; clientId: string; clientSecret: string }
...
function connect(conn: Connection) {
  if (conn.type === 'linkedin-ads') {
    // TypeScript knows conn has linkedin-ads-specific properties
  }
}
```
...
{
  "oneOf": [
    { "$ref": "./linkedin-ads-connection.schema.json" },
...
```json
{
  "type": "LinkedIn Ads" // ❌ Case mismatch
}
```

**Result**: Validation error - "Type must be exactly 'linkedin-ads'"

```json
{
  "type": "linkedin-ads-connection" // ❌ Wrong format
}
```
