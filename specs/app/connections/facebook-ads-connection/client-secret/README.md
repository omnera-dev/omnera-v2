...
Confidential credential used to authenticate the application during OAuth 2.0 token exchange with Facebook Ads. This secret proves the application's identity and must be kept secure at all times.
...
```json
{
  "clientSecret": "secret_facebook-ads_abc123xyz789"
}
```
...
- **Process**:
  1. Generate new secret in Facebook Ads OAuth settings
  2. Update Omnera connection with new secret
  3. Test OAuth flow with new credentials
  4. Revoke old secret in Facebook Ads
...
```http
POST https://facebook-ads.com/oauth2/v1/token
Content-Type: application/x-www-form-urlencoded

grant_type=authorization_code
&code=AUTH_CODE_FROM_CALLBACK
&client_id=facebook-ads_client_abc123
&client_secret=secret_facebook-ads_xyz789
...
```http
POST /oauth/token
client_id=facebook-ads_client_123
