...
- **Value**: Must be exactly `"airtable"` (case-sensitive)
- **Required**: Yes
...
switch (connection.type) {
  case 'airtable':
    return new AirtableOAuthHandler(connection)
  case 'notion':
...
```typescript
type Connection =
  | { type: 'airtable'; clientId: string; clientSecret: string }
...
function connect(conn: Connection) {
  if (conn.type === 'airtable') {
    // TypeScript knows conn has airtable-specific properties
  }
}
```
...
{
  "oneOf": [
    { "$ref": "./airtable-connection.schema.json" },
...
```json
{
  "type": "Airtable" // ❌ Case mismatch
}
```

**Result**: Validation error - "Type must be exactly 'airtable'"

```json
{
  "type": "airtable-connection" // ❌ Wrong format
}
```
