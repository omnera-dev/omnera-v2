 ...
 *
 * If the app has custom pages defined, renders the first page with its sections.
 * Otherwise, falls back to the DefaultHomePage.
 *
...
export function renderHomePage(app: App): string {
  // Check if app has custom pages and render the first one (homepage)
  const pages = app.pages
  if (pages && pages.length > 0) {
    const homePage = pages[0] as typeof pages[number]
    if (homePage) {
  ...
  // Fall back to default homepage if no custom pages
