...
const authMiddleware: Middleware = () => {
  return async (ctx, next) => {
    ...
    if (ctx.data.betterAuthSetCookie) {
      ...
      ctx.res.headers['set-cookie'] = [...origSetCookie, ctx.data.betterAuthSetCookie as string]
    }
  }
}
...
export function getSession(): Promise<Session | null> {
  ...
  const existingSessionPromise = contextData.sessionPromise as Promise<Session | null> | undefined
